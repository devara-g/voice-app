...
export default function VoiceChat({ 
  ...
  useEffect(() => {
    ...
    // Global handler untuk menangkap Promise Rejection dari LiveKit SDK (misal: PublishTrackError)
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      ...
      if (reason && (
        ...
        event.preventDefault(); // Mencegah crash screen Next.js di mode development
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <LiveKitRoom
        ...
        video={false}
        onDisconnected={() => {
          setError('Koneksi terputus dari server suara.');
        }}
        onError={(err) => {
    ...
    <div className="h-full flex flex-col bg-[#1e1f22]">
      ...
      <div className="bg-[#2b2d31] p-4 flex justify-between items-center border-b border-[#1e1f22] shadow-md">
        <div className="flex items-center gap-3">
          <div className="bg-[#35373d] p-2 rounded-lg text-[#23a559]">
            🎙️
          </div>
          <div>
            ...
            <p className="text-gray-400 text-xs">Server: {serverId}</p>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative mb-3">
                    {hasPreset || !pAvatar ? (
                      <div className={`w-20 h-20 rounded-full bg-gradient-to-tr ${getPresetClasses(pAvatar || 'preset:purple')} flex items-center justify-center text-white text-2xl font-bold transition-all duration-300 ${
                        isSpeaking 
                    ...
                    {/* Badge Mikrofon */}
                    <div className={`absolute bottom-0 right-0 w-7 h-7 rounded-full flex items-center justify-center border-2 border-[#2b2d31] shadow-md transition-all duration-300 ${
                      isMuted 
      ...
      {/* Room Audio Renderer - Penting untuk suara masuk */}
      <RoomAudioRenderer />
    </div>
  );
}