...
import { Plus, Compass, Users, Volume2, ChevronRight, LogOut as LeaveIcon, Link, Mic, PhoneOff, Menu, Settings, Hash } from 'lucide-react';
...
export default function ServerDetail() {
  ...
  useEffect(() => {
    ...
    const createDefaultChannel = async () => {
      // Buat channel teks 'umum' dan voice 'General' secara bersamaan
      await Promise.all([
        supabase.from('channels').insert([{ server_id: serverId, name: 'umum', type: 'text' }]),
        supabase.from('channels').insert([{ server_id: serverId, name: 'General', type: 'voice' }]),
      ]);
      ...
      if (data) {
        setChannels(data);
        const firstText = data.find((c: Channel) => c.type === 'text');
  ...
  return (
    <div className="h-screen bg-[#1e1f22] flex relative overflow-hidden">
      ...
      <div className={`fixed md:relative md:flex z-40 top-0 bottom-0 left-0 flex transition-transform duration-300 flex-shrink-0 h-full ${isLeftSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}>
        ...
        <div className="w-[72px] bg-[#1e1f22] flex flex-col items-center py-3 gap-2 border-r border-[#2b2d31] flex-shrink-0 h-full">
          ...
          {servers.map((s) => {
            ...
            return (
              ...
              >
                {/* Pill Indicator */}
                <div className={`absolute left-0 w-1 bg-white rounded-r-md transition-all duration-200 origin-left ${isActive ? 'scale-y-100 h-10' : 'scale-y-0 group-hover:scale-y-[0.6] h-8'
                  }`} />
                ...
                {s.image_url ? (
                  ...
                  <div className={`w-12 h-12 rounded-2xl hover:rounded-xl transition-all flex items-center justify-center text-white font-bold shadow-md ${isActive
              ...
              <div className="flex-1 p-3 overflow-y-auto custom-scrollbar">
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between px-2 mb-2 group">
                    <span className="text-slate-400 text-xs font-extrabold uppercase tracking-widest hover:text-slate-300 cursor-pointer flex items-center gap-1">
                      <ChevronRight className="w-3 h-3 transition-transform rotate-90" />
                      Saluran Teks
                    </span>
                    <button
                      onClick={() => setIsCreateChannelOpen(true)}
                      className="text-slate-400 hover:text-white transition rounded p-0.5 hover:bg-[#35373d] opacity-0 group-hover:opacity-100"
                      title="Buat Saluran"
                    >
                  ...
                  <div className="space-y-0.5">
                    {channels.filter(c => c.type === 'text').map((channel) => {
                      const isActive = activeTextChannel?.id === channel.id;
                      return (
                        <div
                          key={channel.id}
                          onClick={() => setActiveTextChannel({ id: channel.id, name: channel.name })}
                          className={`flex items-center gap-2.5 px-3 py-1.5 cursor-pointer rounded-md transition-all ${isActive
                            ? 'bg-[#35373d]/80 text-white'
                            : 'text-slate-400 hover:bg-[#35373d]/50 hover:text-slate-200'
                            }`}
                        >
                      ...
                      return (
                        <div key={channel.id} className="rounded-lg overflow-hidden">
                          ...
                          <div
                            ...
                            className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer group transition-all ${isActive
                              ? 'bg-[#35373d] text-white'
                              : 'hover:bg-[#35373d] text-slate-300 hover:text-white'
                              }`}
                          >
                            <Volume2 className={`w-4 h-4 flex-shrink-0 ${participants.length > 0 ? 'text-green-400' : isActive ? 'text-indigo-400' : 'text-slate-500 group-hover:text-slate-300'
                              }`} />
                              ...
                              {participants.map((p) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className={`text-xs truncate max-w-[90px] ${isMe ? 'text-indigo-300 font-semibold' : 'text-slate-400'
                                      }`}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex-1 flex flex-col bg-[#313338] min-w-0 min-h-0">
                  ...
                  <span className="text-white font-extrabold text-sm flex items-center gap-1.5 uppercase tracking-wider truncate">
                    {activeTextChannel ? (
                      <>
                        ...
                        <span className="truncate">{activeTextChannel.name}</span>
                      </>
                    ) : (
                      <>
             ...
             <div className="flex-1 flex flex-col min-h-0 bg-[#313338] min-w-0 overflow-hidden">
  {activeTextChannel ? (
    <TextChat
    ...
    />
                ...
                <div className="flex-1 flex flex-col bg-[#313338] min-w-0 min-h-0 overflow-hidden">
                    <div className="max-w-sm">
                      ...
                      <p className="text-slate-400 text-sm mt-2">
                        Pilih salah satu saluran suara di menu kiri untuk terhubung dan mulai mengobrol secara langsung dengan anggota server lainnya, atau pilih saluran teks untuk mulai mengetik.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className={`fixed md:relative md:flex z-35 top-0 bottom-0 right-0 w-60 bg-[#2b2d31] border-l border-[#1e1f22]/30 flex-shrink-0 h-full flex flex-col transition-transform duration-300 ${isRightSidebarOpen ? 'translate-x-0' : 'translate-x-full md:translate-x-0'
              }`}>
